...
@Component
public class DbIssueVoidEventTestData extends com.bfds.saec.batch.util.DataGenerator {
    ...
    public void create() {
        ...
        // We will assign new Checks without check# to this claimant.
        // We can then use the claimant to get hold of all these checks.
        claimant = newClaimant();
        claimant.setReferenceNo("100000002");
        createNewChecksWithoutCheckNo(claimant);
        claimant.persist();
        claimant.flush();
        claimant.clear();
    }
    ...
    /**
     * Creates Checks in the rCreated state/status. The check will not have a check #.
     *
    ...
    /**
     * Creates Checks in the re-issue approved state/status.
     *
     * @param claimant The {@link Claimant} to which the checks are assigned.
     */
    ...
    private Payment newPayment(Claimant claimant, String checkNo, PaymentCode paymentCode, PaymentCalc paymentCalc) {
        ...
        if(paymentCode == PaymentCode.FIRST_ISSUE_CREATED_FI_FI) {
            c.setTranch(Tranch.findByCode("t1"));
        }
        c.setPayTo(claimant);
        claimant.addCheck(c);
        return c;
